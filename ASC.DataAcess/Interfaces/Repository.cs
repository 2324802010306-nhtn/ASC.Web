...
namespace ASC.DataAccess.Interfaces
{
    ...
    public class Repository<T> : IRepository<T> where T : BaseEntity, new()
    {
        ...
        public async Task<IEnumerable<T>> FindAllByPartitionKeyAsync(string partitionkey)
        {
            ...
            var result = dbContext.Set<T>().Where(t => t.PartitionKey == partitionkey).ToListAsync().Result; return result as IEnumerable<T>;
        }
        public async Task<IEnumerable<T>> FindAllAsync()
        {
            var result = dbContext.Set<T>().ToListAsync().Result; return result as IEnumerable<T>;
        }
    }
}